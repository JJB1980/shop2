...
while ($r1 = sql\row($res1)) {
        $cat1 = array();
        $cat1['CategoryCode'] = $r1['CategoryCode'];
        $cat1['CategoryDescription'] = $r1['CategoryDescription'];
        $cat1['SubCat1'] = array();
       	$res2 = sql\dqry("select * from SubCategories1 where CategoryCode = '".$r1['CategoryCode']."'");
        $count1 = 0;
	while ($r2 = sql\row($res2)) {
            $cat2 = array();
            $cat2['SubCategory1Code'] = $r2['SubCategory1Code'];
            $cat2['SubCategory1Description'] = $r2['SubCategory1Description'];
            $res3 = sql\dqry("select * from SubCategories2 where SubCategory1Code = '".$r2['SubCategory1Code']."'");
            $cat2['SubCat2'] = array();
            $count2 = 0;
            while ($r3 = sql\row($res3)) {
               $cat3 = array();
